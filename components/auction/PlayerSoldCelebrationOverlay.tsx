  ...
  return createPortal(
    <AnimatePresence mode="wait">
      ...
      >
        ...
        <motion.div
          className="fixed left-1/2 top-1/2 overflow-hidden rounded-[1.6rem] border border-white/15 bg-black/80 px-4 py-5 shadow-[0_24px_100px_rgba(0,0,0,0.68)] backdrop-blur-2xl sm:rounded-[2rem] sm:px-5 sm:py-6 md:px-8 md:py-8"
          style={{
            width: `${layout.cardWidth}px`,
            maxHeight: `${layout.cardMaxHeight}px`,
          }}
          initial={{ scale: 0.72, opacity: 0, x: '-50%', y: '-50%' }}
          animate={reduceMotion ? { scale: 1, opacity: 1, x: '-50%', y: '-50%' } : { scale: 1, opacity: 1, x: '-50%', y: '-50%' }}
