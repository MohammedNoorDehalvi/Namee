  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 px-4 py-3 transition-all duration-300">
      <div className="max-w-7xl mx-auto">
        <nav
          className="flex items-center justify-between h-14 px-4 md:px-6 rounded-full backdrop-blur-xl bg-slate-950/75 border border-white/10 shadow-2xl shadow-black/50"
        ...
        <AnimatePresence>
          {open && (
            <motion.div
              ...
              className="lg:hidden mt-2 p-4 rounded-2xl backdrop-blur-2xl bg-slate-950/95 border border-white/10 shadow-2xl space-y-2"
            >
              {links.map((link) => (
                <Link
                  key={link.href}
                  ...
                  className="flex items-center justify-between p-3 rounded-xl hover:bg-white/5 text-slate-200 hover:text-white font-medium text-sm transition-colors"
                >
              ...
              <div className="pt-2 border-t border-white/10">
                {session ? (
                  <button
                    ...
                    className="w-full text-center py-2.5 rounded-xl bg-slate-800 hover:bg-slate-700 text-slate-200 font-semibold text-sm transition-colors"
                  >
                    Logout ({session.name})
                  </button>
                ) : (
                  <Link
                    href="/captain-login"
                    onClick={() => setOpen(false)}
                    className="flex items-center justify-center gap-2 w-full py-2.5 rounded-xl bg-gradient-to-r from-amber-500 to-amber-600 text-slate-950 font-bold text-sm shadow-lg shadow-amber-500/20"
                  >
                    <span>Captain Login</span>
