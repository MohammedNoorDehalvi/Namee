...
-- Safe migration: does not delete any existing data.
...
-- 2) Add season_id safely to current tables.
...
declare
  active_id uuid;
begin
  select id into active_id from public.seasons where status = 'active' order by started_at desc limit 1;
...
-- 4) Bid table missing columns used by newer routes.
...
-- 5) Optional season-aware match/points placeholder tables for old-season read-only pages.
